...
class ModelResultsCollectorToSave(CallbackBase):  
    ...
    def __init__(self, redisKey,mysqlKey=None,*args, **kwargs):
    ...
    def save_msg(self,data):
        if self.redisKey and self.mysqlKey:
            DsRedis.OpsAnsibleModel.lpush(self.redisKey,data)  
            DeploySaveResult.Model.insert(self.mysqlKey, data)
...
class PlayBookResultsCollectorToSave(CallbackBase):  
    ...
    def save_msg(self,msg):
        if self.redisKey and self.mysqlKey:
            ...
            DeploySaveResult.PlayBook.insert(self.mysqlKey, msg)
    ...
    def v2_playbook_on_stats(self, stats):
        msg = "<font color='#FFFFFF'>\nPLAY RECAP *********************************************************************</font>"
        DsRedis.OpsAnsiblePlayBook.lpush(self.redisKey,msg)
        if self.mysqlKey:DeploySaveResult.PlayBook.insert(self.mysqlKey, msg)
        hosts = sorted(stats.processed.keys())
        for h in hosts:
            t = stats.summarize(h)
            self.task_status[h] = {
...
def AdHoccallback(redisKey,mysqlKey):
    if redisKey or mysqlKey :
        return ModelResultsCollectorToSave(redisKey,mysqlKey) 
    else:
        return ModelResultsCollector()
    
def Playbookcallback(redisKey,mysqlKey):
    if redisKey or mysqlKey :
        return PlayBookResultsCollectorToSave(redisKey,mysqlKey) 
    else:
