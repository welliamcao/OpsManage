...
def db_detail(request, id,format=None):
    ...
    if request.method == 'PUT':
        serializer = DataBaseServerSerializer(snippet, data=request.data)
        if serializer.is_valid():
            serializer.save()
...
def inc_list(request,format=None):
    ...
    """     
    if request.method == 'POST':
        serializer = InceptionSerializer(data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data, status=status.HTTP_201_CREATED)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
def inc_detail(request, id,format=None):
    ...
    try:
        ...
        return Response(status=status.HTTP_404_NOT_FOUND)
    if request.method == 'PUT':
        serializer = InceptionSerializer(snippet, data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
def sql_custom_list(request,format=None):
    ...
    """     
    if request.method == 'POST':
        serializer = CustomSQLSerializer(data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data, status=status.HTTP_201_CREATED)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
def sql_custom_detail(request, id,format=None):
    ...
    try:
        ...
        return Response(status=status.HTTP_404_NOT_FOUND)
    if request.method == 'PUT':
        serializer = CustomSQLSerializer(snippet, data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
