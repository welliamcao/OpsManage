...
from dao.account import UsersManage,RolesManage
...
class UserManage(LoginRequiredMixin,UsersManage,View):
    ...
    @method_decorator_adaptor(permission_required, "account.account_read_user","/403/")  
    ...
    @method_decorator_adaptor(permission_required, "account.account_change_user","/403/") 
...
class RoleManage(LoginRequiredMixin,RolesManage,View):
    login_url = '/login/'
    def get(self, request, *args, **kwagrs):
        if request.GET.get('type'):
            res = self.allowcator(request.GET.get('type'), request)
